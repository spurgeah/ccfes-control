...
params = {
    "amp1": 5,  # Amplitude for channel 1 (mA)
    "amp2": 5,  # Amplitude for channel 2 (mA)
    "freq1": 20,  # Frequency for channel 1 (Hz)
    "freq2": 20,  # Frequency for channel 2 (Hz)
    "pw1": 360,  # Pulse width for channel 1 (microseconds)
    "pw2": 360   # Pulse width for channel 2 (microseconds)
}
...
# ADC / conversion parameters (matches device setup)
VREF = 4.0  # Volts
GAIN = 6    # Amplifier gain used on the device
CURRENT = 30e-6  # 30 µA injected (used for converted BI channel)
...
# Normalization / thresholds (user should set MVC values per muscle in millivolts)
MVC_CH1_UV = 30 # mV -> .004 µV
MVC_CH2_UV = 60 # mV -> .004 µV
THRESHOLD_NORM = 0.20   # normalized threshold as fraction of MVC (e.g., 0.10 -> 10% MVC)
...
def build_stim_config_off():
    """Build OFF stimulation configuration (zero amplitude)."""
    # Zero amplitude configuration to turn off stimulation
    c1 = [
        ChannelPoint(int(params["pw1"] / 2), 0),
        ChannelPoint(int(params["pw1"] / 2), 0),
        ChannelPoint(int(params["pw1"] / 2), 0)
    ...
    # Create OFF configuration objects for both channels
    cc1_off = MidLevelChannelConfiguration(True, 3, params["freq1"], c1)
    cc2_off = MidLevelChannelConfiguration(True, 3, params["freq2"], c2)
    return [cc1_off, cc2_off]
...
async def main():
  ...
  # --- Setup P24 Stimulator ---
    ...
    # Track FES ON/OFF state for each channel to avoid repeated commands
    fes_ch1_is_on = False
    fes_ch2_is_on = False
        ...
        while not stop_event.is_set():
            ...
            if ack and ack.command == Commands.DL_SEND_LIVE_DATA:
                ...
                if len(buffer_ch1) >= block_size:
                    ...
                    # Threshold detection and FES control
                    # For CH1: Turn FES ON when effort >= THRESHOLD_NORM, OFF when drops below
                    if norm_ch1 >= THRESHOLD_NORM:
                        # Effort is above threshold - turn FES ON (if not already on)
                        if not fes_ch1_is_on:
                            print(f"[CH1] Effort above threshold ({norm_ch1*100:.4f}% MVC). Turning FES ON.")
                            try:
                                configs_on = build_stim_config()
                                await stim_layer.update(configs_on)
                                fes_ch1_is_on = True
                            except Exception as e:
                                print(f"[CH1] Error turning FES ON: {e}")
                        else:
                            # FES already on, just print status
                            print(f"[CH1] FES ON: {norm_ch1*100:.4f}% MVC (>= {THRESHOLD_NORM*100:.4f}%)")
                    else:
                        # Effort is below threshold - turn FES OFF (if currently on)
                        if fes_ch1_is_on:
                            print(f"[CH1] Effort dropped below threshold ({norm_ch1*100:.4f}% MVC). Turning FES OFF.")
                            try:
                                configs_off = build_stim_config_off()
                                await stim_layer.update(configs_off)
                                fes_ch1_is_on = False
                            except Exception as e:
                                print(f"[CH1] Error turning FES OFF: {e}")
                        else:
                            # FES already off, just print status
                            print(f"[CH1] FES OFF: {norm_ch1*100:.4f}% MVC (< {THRESHOLD_NORM*100:.4f}%)")

                    # For CH2 (optional - can be configured similarly or left for future use)
                    if norm_ch2 >= THRESHOLD_NORM:
                        print(f"[CH2] Above threshold: {norm_ch2*100:.4f}% MVC")
                    else:
                        print(f"[CH2] Below threshold: {norm_ch2*100:.4f}% MVC")
