...
emg_threshold = .2  # Default EMG threshold for turning on stimulation (can be changed by user)
...
async def main():
    ...
    CURRENT = 30e-6  # 30 uA injected - page 15 ads129 manual
    ...
    plot_helper = FastPlotLibHelper({
        0: ["EMG CH1", "g"],   # EMG channel 1, green
        1: ["EMG CH2", "m"],   # EMG channel 2, magenta
        2: ["Stim Amp1", "r"], # Stimulation amplitude channel 1, red
        3: ["Stim Amp2", "b"]  # Stimulation amplitude channel 2, blue
    ...
    while not stop_event.is_set():
        ...
        if ack and ack.command == Commands.DL_SEND_LIVE_DATA:
            ...
            # converted[1] = ch 2, converted[2] = ch 3
            c1 = [
                ChannelPoint(int(params["pw1"] / 2), int(params["amp1"])) if converted[1] > emg_threshold else ChannelPoint(int(params["pw1"] / 2), 0),
                ChannelPoint(int(params["pw1"] / 2), 0),
                ChannelPoint(int(params["pw1"] / 2), -int(params["amp1"])) if converted[1] > emg_threshold else ChannelPoint(int(params["pw1"] / 2), 0)
            ]
            c2 = [
                ChannelPoint(int(params["pw2"] / 2), int(params["amp2"])) if converted[2] > emg_threshold else ChannelPoint(int(params["pw2"] / 2), 0),
                ChannelPoint(int(params["pw2"] / 2), 0),
                ChannelPoint(int(params["pw2"] / 2), -int(params["amp2"])) if converted[2] > emg_threshold else ChannelPoint(int(params["pw2"] / 2), 0)
            ]
            cc1 = MidLevelChannelConfiguration(True, 3, params["freq1"], c1)
            ...
            csv_row = [
                ...
                *converted,               # All filtered EMG channels
                params["amp1"] if stim_on_ch1 else 0,  # stim_amp1
                params["freq1"],                 # stim_freq1
                params["pw1"],                   # stim_pw1
                params["amp2"] if stim_on_ch2 else 0,  # stim_amp2
