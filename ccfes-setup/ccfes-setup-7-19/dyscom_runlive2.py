...
def main():
    ...
    def data_acquisition():
        async def run():
            ...
            await dyscom.power_module(DyscomPowerModuleType.MEASUREMENT, DyscomPowerModulePowerType.SWITCH_ON)  # Turns on measurement mode

            # Set up what kind of signals we want (e.g., EMG, breathing)
            init_params = DyscomInitParams()
            init_params.signal_type = [
                DyscomSignalType.BI,  # Bio-impedance
                DyscomSignalType.EMG_1,
                DyscomSignalType.EMG_2,
                DyscomSignalType.BREATHING
            ]
            # Set resolution and speed of data collection
            ...
            await dyscom.init(init_params)  # Initializes the device with these settings
            await dyscom.start()  # Starts sending live data

            try:
                while not stop_event.is_set():  # Keep collecting until stop signal
                    ack = dyscom.packet_buffer.get_packet_from_buffer()  # Get latest data
                    if ack and ack.command == Commands.DL_SEND_LIVE_DATA:
                        ...
                        plot_helper.update()
                        # Save all 5 channels plus time info to CSV
                        ...
                        await asyncio.sleep(0.001)  # Wait a tiny bit before checking again (reduces CPU use)
            finally:
                await dyscom.stop()  # Stop data streaming
                await dyscom.power_module(DyscomPowerModuleType.MEASUREMENT, DyscomPowerModulePowerType.SWITCH_OFF)  # Turn off device power

        asyncio.run(run())  # Runs the data acquisition loop asynchronously
