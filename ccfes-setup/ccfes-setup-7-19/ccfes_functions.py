...
class FastPlotLibHelper(PlotHelper):
    ...
    #def __init__(self, channels: dict[int, tuple[str, str]], max_value_count: int):
        ...
        def set_text_overlay(self, text: str):
                """Add or update a text overlay on the figure."""
                import numpy as np
                if self._text_overlay is None:
                    # Add text to the first subplot (upper left)
                    subplot = list(self._figure)[0]
                    self._text_overlay = subplot.add_text(
                        text,
                        position=(0.01, 0.95),  # top-left, normalized coordinates
                        anchor="topleft",
                        color="black",
                        font_size=18,
                        ndc=True
